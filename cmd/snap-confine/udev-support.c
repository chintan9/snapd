...
static void _run_snappy_app_dev_add_majmin(struct snappy_udev *udev_s, const char *path, unsigned major,
                                           unsigned minor) {
    int status = 0;
    pid_t pid = fork();
    if (pid < 0) {
    ...
    if (pid == 0) {
        ...
        if (getresuid(&real_uid, &effective_uid, &saved_uid) != 0) die("cannot get real, effective and saved user IDs");
        // can't update the cgroup unless the real_uid is 0, euid as
        // 0 is not enough
        if (real_uid != 0 && effective_uid == 0)
            if (setuid(0) != 0) die("cannot set user ID to zero");
        char buf[64] = {0};
        ...
        debug("running snap-device-helper add %s %s %s", udev_s->tagname, path, buf);
        ...
        if (access("/usr/lib/snapd/snap-device-helper", X_OK) == 0)
            execle("/usr/lib/snapd/snap-device-helper", "/usr/lib/snapd/snap-device-helper", "add", udev_s->tagname,
                   path, buf, NULL, env);
        else if (access("/usr/libexec/snapd/snap-device-helper", X_OK) == 0)
            execle("/usr/libexec/snapd/snap-device-helper", "/usr/libexec/snapd/snap-device-helper", "add",
                   udev_s->tagname, path, buf, NULL, env);
        else
            execle("/lib/udev/snappy-app-dev", "/lib/udev/snappy-app-dev", "add", udev_s->tagname, path, buf, NULL,
                   env);
        die("execl failed");
    }
...
void run_snappy_app_dev_add(struct snappy_udev *udev_s, const char *path) {
    if (udev_s == NULL) die("snappy_udev is NULL");
    if (udev_s->udev == NULL) die("snappy_udev->udev is NULL");
    if (udev_s->tagname_len == 0 || udev_s->tagname_len >= MAX_BUF ||
        strnlen(udev_s->tagname, MAX_BUF) != udev_s->tagname_len || udev_s->tagname[udev_s->tagname_len] != '\0')
        die("snappy_udev->tagname has invalid length");

    debug("%s: %s %s", __func__, path, udev_s->tagname);

    struct udev_device *d = udev_device_new_from_syspath(udev_s->udev, path);
    if (d == NULL) die("cannot find device from syspath %s", path);
...
int snappy_udev_init(const char *security_tag, struct snappy_udev *udev_s) {
    ...
    udev_s->tagname_len = sc_must_snprintf(udev_s->tagname, MAX_BUF, "%s", security_tag);
    for (size_t i = 0; i < udev_s->tagname_len; i++)
        if (udev_s->tagname[i] == '.') udev_s->tagname[i] = '_';

    udev_s->udev = udev_new();
    if (udev_s->udev == NULL) die("udev_new failed");

    udev_s->devices = udev_enumerate_new(udev_s->udev);
    if (udev_s->devices == NULL) die("udev_enumerate_new failed");

    if (udev_enumerate_add_match_tag(udev_s->devices, udev_s->tagname) < 0) die("udev_enumerate_add_match_tag");

    if (udev_enumerate_scan_devices(udev_s->devices) < 0) die("udev_enumerate_scan failed");

    udev_s->assigned = udev_enumerate_get_list_entry(udev_s->devices);
    if (udev_s->assigned == NULL) rc = -1;
...
void snappy_udev_cleanup(struct snappy_udev *udev_s) {
    ...
    if (udev_s->devices != NULL) udev_enumerate_unref(udev_s->devices);
    if (udev_s->udev != NULL) udev_unref(udev_s->udev);
}
...
void setup_devices_cgroup(const char *security_tag, struct snappy_udev *udev_s) {
    ...
    const char *static_devices[] = {
        "/sys/class/mem/null",    "/sys/class/mem/full",    "/sys/class/mem/zero",
        "/sys/class/mem/random",  "/sys/class/mem/urandom", "/sys/class/tty/tty",
        "/sys/class/tty/console", "/sys/class/tty/ptmx",    NULL,
    };

    if (udev_s == NULL) die("snappy_udev is NULL");
    if (udev_s->udev == NULL) die("snappy_udev->udev is NULL");
    if (udev_s->devices == NULL) die("snappy_udev->devices is NULL");
    if (udev_s->assigned == NULL) die("snappy_udev->assigned is NULL");
    if (udev_s->tagname_len == 0 || udev_s->tagname_len >= MAX_BUF ||
        strnlen(udev_s->tagname, MAX_BUF) != udev_s->tagname_len || udev_s->tagname[udev_s->tagname_len] != '\0')
        die("snappy_udev->tagname has invalid length");
    ...
    sc_must_snprintf(cgroup_dir, sizeof(cgroup_dir), "/sys/fs/cgroup/devices/%s/", security_tag);
    sc_identity old = sc_set_effective_identity(sc_root_group_identity());
    if (mkdir(cgroup_dir, 0755) < 0 && errno != EEXIST) die("cannot create cgroup hierarchy %s", cgroup_dir);
    (void)sc_set_effective_identity(old);

    // move ourselves into it
    char cgroup_file[PATH_MAX] = {0};
    sc_must_snprintf(cgroup_file, sizeof(cgroup_file), "%s%s", cgroup_dir, "cgroup.procs");
    ...
    sc_must_snprintf(cgroup_file, sizeof(cgroup_file), "%s%s", cgroup_dir, "devices.deny");
    write_string_to_file(cgroup_file, "a");

    // add the common devices
    for (int i = 0; static_devices[i] != NULL; i++) run_snappy_app_dev_add(udev_s, static_devices[i]);
    ...
    for (unsigned pty_major = 136; pty_major <= 143; pty_major++) {
        ...
        // addition is for
        _run_snappy_app_dev_add_majmin(udev_s, "/dev/pts/slaves", pty_major, UINT_MAX);
    }
    ...
    for (unsigned nv_minor = 0; nv_minor < 255; nv_minor++) {
        ...
        _run_snappy_app_dev_add_majmin(udev_s, nv_path, major(sbuf.st_rdev), minor(sbuf.st_rdev));
    }

    // /dev/nvidiactl
    if (stat(nvctl_path, &sbuf) == 0) {
        _run_snappy_app_dev_add_majmin(udev_s, nvctl_path, major(sbuf.st_rdev), minor(sbuf.st_rdev));
    }
    // /dev/nvidia-uvm
    if (stat(nvuvm_path, &sbuf) == 0) {
        _run_snappy_app_dev_add_majmin(udev_s, nvuvm_path, major(sbuf.st_rdev), minor(sbuf.st_rdev));
    }
    // /dev/nvidia-modeset
    if (stat(nvidia_modeset_path, &sbuf) == 0) {
        _run_snappy_app_dev_add_majmin(udev_s, nvidia_modeset_path, major(sbuf.st_rdev), minor(sbuf.st_rdev));
    }
    ...
    if (stat("/dev/uhid", &sbuf) == 0) {
        _run_snappy_app_dev_add_majmin(udev_s, "/dev/uhid", major(sbuf.st_rdev), minor(sbuf.st_rdev));
    }
    // add the assigned devices
    while (udev_s->assigned != NULL) {
        const char *path = udev_list_entry_get_name(udev_s->assigned);
        if (path == NULL) die("udev_list_entry_get_name failed");
